...
|---|---|---|---|---|---|
| TD-001 | `services/order-service` | No integration test for the Kafka publication path | The example service demonstrates the outbox structure; asserting delivery needs a broker container and roughly doubles the suite's wall-clock time | A regression in `KafkaEventPublisher` header handling would not be caught until a consumer complained | Adding a second event, or a second consumer that depends on the headers |
